...
parser.add_argument('--record', help='the path to the record directory: xxx/yyy.bag')
record = False # 后续关闭记录需要用到的标志物
parser.add_argument('--playback', help='the path to the recorded xxx.bag file')
playback = False
...
# 实时发布检测到的人位置信息
def pub_people_position(pub, x, y, z):
    # 设置发布频率
    rate = rospy.Rate(10)
    # 创建一个相机坐标系中的坐标点
...
if __name__ == '__main__':
    ...
    pub = rospy.Publisher("/guide_dog_v0.1/robo_vision/people_position_camera", PointStamped, queue_size=10)
    try:
        while True:
            ...
            if p_xyz_list:
                ...
                print(*p_xyz_list[max_conf_index])
                pub_people_position(pub, *p_xyz_list[max_conf_index])
